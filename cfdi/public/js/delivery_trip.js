frappe.ui.form.on("Delivery Trip", {
	...
	timbrar: function(frm,dt,dn) {
		frappe.call({
			...
			callback: function (data) {
      	carta_porte_timbrado(frm,dn,data.message)
      }
    })
	},
...
var carta_porte_timbrado = function(frm,dn,v){
  frappe.call({
          method: "cfdi.cfdi.doctype.cfdi.cfdi.carta_porte_timbrado",
          args:{
            url: 'https://' + v.url,
            token: v.token,
            docname: dn,
...
var sales_invoice_cancelar = function(frm,dn,v) {
  frappe.call({
	method: "cfdi.cfdi.doctype.cfdi.cfdi.cancel_by_uuid_carta_porte",
	freeze: true,
	freeze_message: 'Timbrando. Por favor espere...',
	args:{
		url: 'https://' + v.url,
